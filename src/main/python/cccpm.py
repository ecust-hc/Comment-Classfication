...
class What2WriteClassifier:
    ...
    def _feature_transform(text, label):
        ...
        label_encoder.fit(["what", "why", "howtouse","howitisdone", "property", "others"])
    ...
    @staticmethod
    def _feature_y(labels, label_names=["what", "why", "howtouse", "howitisdone", "property", "others"]):
        # label_encoder将类别转换为数字, 它是按照字母序来的
        # inverse_transform可以将其转换
        label_encoder = preprocessing.LabelEncoder()
...
def evaluate_merge(validation_code, validation_label, testing_code, clf='rf'):
    ...
    #新增的代码，主要将预测类型为how to use的代码放在一个文件中
    origin=[]
    with open(TEST_CODE) as f:
        for line in tqdm(f.readlines()):
            origin.append(line.strip())

    # print(pred_label_20000)
    result=[]
    for index, label in enumerate(pred_label_20000):
        if label in ['howtouse']:
            result.append(origin[index])
    file = open("D:\Python\TOSEM_CodeSum\out.txt", "w", encoding="utf8")
    i=0
